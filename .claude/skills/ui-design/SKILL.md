...
이 가이드의 컬러/타이포/컴포넌트 토큰은 `public/deck/`의 Sightline 데모덱 시각 언어를 기준으로 삼는다.

- **인증 후 화면(`(app)` 라우트 — 대시보드/업로드/로그인)은 다크모드 고정.** 매일 확인하는 도구처럼 보여야 한다는 원칙 1 때문에, 여기서는 라이트 섹션을 쓰지 않는다.
- **마케팅 랜딩페이지(`(marketing)` 라우트)는 데크처럼 다크/라이트 섹션을 번갈아 쓴다** (2026-07-27 결정, 사용자 요청 — 전부 다크면 읽기 피로가 커서 섹션 리듬으로 대비를 준다). 실제 제품 화면을 그대로 보여주는 섹션(히어로 지출 요약 미리보기, AI 인사이트 데모)은 실제 앱과 동일하게 **다크로 유지**해 "이게 진짜 화면"이라는 신뢰를 주고, 순수 설명/카피 섹션(작동 방식, 기능 비교, 신뢰 문구, 마지막 CTA)은 **라이트**로 리듬을 만든다. 색 토큰은 아래 "배경(라이트)" 참고.
...
### 배경 (라이트, 마케팅 랜딩 전용 — 위 다크/라이트 섹션 규칙 참고)
| 용도 | 값 |
|------|------|
| 섹션 배경 | #ffffff |
| 카드 (라이트 섹션 안) | #ffffff, `border border-[#dee1e6]` |
| 아이콘 원형 배지 배경(중립, 시맨틱 컬러 안 쓸 때) | #eef0f3 |
...
### 카드/패널
```
다크: rounded-[24px] bg-[#16181c] p-8
라이트(마케팅 랜딩 라이트 섹션 전용): rounded-[24px] bg-white border border-[#dee1e6] p-8
```
다크 표면끼리는 border 없이 배경색 차이만으로 구분한다. `border-[#dee1e6]`는 라이트 섹션의 카드에서만 쓴다(`Card` 컴포넌트의 `tone="light"`).
